...
10/29/15 note that the code is different from what you see in the videos
 A new version of pd was released that is phasing out the convert_objects(convert_numeric=True)
It still works for now, but it is recommended that the pd.to_numeric function be
used instead """ 
...
"""
all_combinations = list(set(combinations(sub2['USFREQMO'], 2)))
all_combinations = list(filter(lambda x: (np.isnan(x[0]) == False) & (np.isnan(x[1]) == False), all_combinations))
